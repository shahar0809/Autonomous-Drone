...
void Dispoint::setDis(std::vector<Point> &vertex){
    ...
    if(point.getX()>=vertex[0].getX()&&point.getX()<=vertex[1].getX()){
        ...
        Dispoint::dis = fmin(fmin(sqrt(std::pow(point.getX()-vertex[0].getX(), 2) + pow(point.getY()-vertex[0].getY(), 2)),
                                  sqrt(std::pow(point.getX()-vertex[1].getX(), 2) + pow(point.getY()-vertex[1].getY(), 2))),
                             fmin(sqrt(std::pow(point.getX()-vertex[2].getX(), 2) + pow(point.getY()-vertex[2].getY(), 2)),
                                  sqrt(std::pow(point.getX()-vertex[3].getX(), 2) + pow(point.getY()-vertex[3].getY(), 2))));
    }
}
